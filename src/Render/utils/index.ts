 ...
 * @Last Modified time: 2021-01-04 16:51:45
 */
/**
 * 比较新旧props达到渲染优化
 ...
 */
export function compareProps<P extends object>(prevProps: React.PropsWithChildren<P>, nextProps: React.PropsWithChildren<P>, renderProps: string[]): boolean {
 ...
 */
export function throttle(func: Function, delay = 120): (...args: unknown[]) => void {
  let prev = Date.now();
  return (...args) => {
    const now = Date.now();
    if (now - prev >= delay) {
 ...
 */
export function debounce(func: Function, wait = 120): (...args: unknown[]) => void {
 ...
 */
export function preventDoublePress(func: () => Promise<void>): () => Promise<void> {
  let isPress = false;
